...
async function checkExistingConnection() {
    ...
    try {
        ...
        if (accounts.length > 0) {
            ...
            // Check for welcome bonus even on existing connection
            const { getBalance, giveWelcomeBonus } = await import('./api.js');
            const balance = await getBalance(userAddress);
            
            if (balance === 0) {
                try {
                    await giveWelcomeBonus(userAddress);
                    alert('Welcome! 🎉 You received 10,000 Braincells as a welcome bonus!');
                } catch (error) {
...
export async function connectWallet() {
    ...
    try {
        ...
        // Check if this is a new user (balance = 0)
        const { getBalance, giveWelcomeBonus } = await import('./api.js');
        const balance = await getBalance(userAddress);
        ...
        if (balance === 0) {
            try {
                ...
                console.error('Error giving welcome bonus:', error);
                // Don't block the connection if bonus fails
            }
        }
...
export async function updateBraincellsBalance() {
    ...
    try {
        const { fetchWithCORS } = await import('./api.js');
        const response = await fetchWithCORS(`https://outskill-cline.onrender.com/balance/${userAddress}`, {
            method: 'GET'
        });
